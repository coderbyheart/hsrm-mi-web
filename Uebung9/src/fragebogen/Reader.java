...
public class Reader {
	...
	private static Document parseXMLFile(InputStream is) {
		...
		try {
			...
			throw new RuntimeException(
					"FragebogenReader::cannot read XML stream");
		}
		return doc;
	}

	/**
	 * Liest Fragebogen-Datei ein und gibt Map von Fragen auf Map von Antwort
	 * auf Punkte zurück.
	 * 
	 * @param is
	 *            Eingabestrom von dem die XML-Datei gelesen wird
	 * @return Fragebogen (Frage-> (Antwort, Punkte)*)*
	 */
	public static SortedMap<String, SortedMap<String, Integer>> getFragebogen(
			InputStream is) {
		...
		for (int i = 0; i < cn.getLength(); i += 1) {
			Node n = cn.item(i);
			String frage = n.getAttributes().getNamedItem("text")
					.getTextContent();
			...
			for (int j = 0; j < nl.getLength(); j += 1) {
				...
				if (a.getNodeName().equals("antwort")) {
					int wert = Integer.parseInt(a.getAttributes()
							.getNamedItem("wert").getTextContent());
	...
	/**
	 * Einfacher Test des Fragebogenreaders. Nicht im Servlet-Container zu
	 * verwenden!
	 * 
	 * @param args
	 */
